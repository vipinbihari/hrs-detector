"""
H2.CL (HTTP/2 to HTTP/1 Content-Length) detector module.

This module implements detection for H2.CL vulnerabilities, which occur when a front-end
server speaks HTTP/2 with clients but downgrades to HTTP/1.1 when forwarding requests
to a back-end server, and the back-end server processes Content-Length headers
...
async def test_h2_cl_vulnerability(
    url: str,
    ...
    """Test for H2.CL HTTP request smuggling vulnerability.
    
    This test detects if a server is vulnerable to H2.CL request smuggling by:
    1. Sending an HTTP/2 request with a Content-Length header
    2. Sending less data than specified in the Content-Length header
    ...
    if verbose:
        logger.info("==== H2.CL TEST PARAMETERS ====")
    ...
    try:
        ...
        mutations = [
            # 1. Normal header
            {
                "description": "Standard Content-Length header",
        ...
        for mutation in mutations:
            ...
            try:
                ...
                try:
                    ...
                    # Here we're testing Content-Length vulnerabilities
                    body_content = "abc"  # lowercase, as requested for proper testing
                    
                    if payload_placement == "normal_header":
                        # Add Content-Length header with a value larger than the actual body
                        ...
                        if verbose:
                            ...
                            logger.info(f"Content-Length header value: {header_value} (should be > {len(body_content)})")
                        
                        # The key part of the test: if the backend is using the
                        # Content-Length from our request but we're sending less data,
                        # it should wait for the rest of the data, causing a timeout
...
async def test_h2_cl(
    ...
    results = await test_h2_cl_vulnerability(
        url=url,
        headers=headers,
        timeout=timeout,
        verbose=verbose,
    ...
    for finding in results.get('findings', []):
        if finding.get('is_vulnerable', False):  
            finding_details = {
                ...
                'details': 'HTTP/2 to HTTP/1 Content-Length desync vulnerability',
...
async def main(args: argparse.Namespace) -> int:
    """Main function for the H2.CL detector.
    ...
    logger.info(f"Testing {args.url} for H2.CL vulnerabilities...")
    ...
    try:
        ...
        try:
            ...
            if findings:
                ...
                logger.warning("VULNERABLE: The target is vulnerable to H2.CL request smuggling!")
                return 1
            else:
...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
        Parsed arguments
    """
    parser = argparse.ArgumentParser(description="Test for H2.CL HTTP request smuggling vulnerabilities")
